...
var IWFeed = Class.create({
  initialize: function (url) {
    if (url) {
      if (IWAllFeeds.hasOwnProperty(url)) {
        iWLog(
          "warning -- use IWCreateFeed rather than new IWFeed and you'll get better performance"
        );
      }
      this.mURL = url;
      this.mLoading = false;
  ...
  p_applyEntryOrder: function (imageStream, entryGUIDs) {
    ...
    (function () {
      return orderedStream.length == entryGUIDs.length;
    }).assert();
...
var IWStreamEntry = Class.create({
  ...
  badgeMarkupForRect: function (rect) {
    return IWStreamEntryBadgeMarkup(
      rect,
      this.isMovie(),
      this.showCommentIndicator() ? this.commentGUID() : null
    );
  },
  thumbnailMarkupForRect: function (rect) {
    return (
      imgMarkup(
        this.thumbnail().sourceURL(),
        rect.position(),
        "",
        this.mTitle
...
function IWStreamEntryBadgeMarkup(rect, isMovie, commentGUID) {
  ...
  var badgeRect = new IWRect(
    rect.origin.x,
    rect.maxY() - kBadgeHeight,
    rect.size.width,
    kBadgeHeight
  );
  var markup = "";
  ...
  if (isMovie) {
    ...
    markup += imgMarkup(
      movieImage.sourceURL(),
      movieRect.position(),
      'class="badge-overlay"'
    );
  }
  var commentLocation = new IWPoint(
    badgeRect.maxX() - kBadgeWidth,
    badgeRect.origin.y
  );
  if (commentGUID) {
    ...
    markup += imgMarkup(
      commentImage.sourceURL(),
      commentRect.position() + "display: none; ",
      'class="badge-overlay" id="comment-badge-' + commentGUID + '"'
    );
  }
  return markup;
}
var IWCommentableStreamEntry = Class.create(IWStreamEntry, {
  ...
  didInsertThumbnailMarkupIntoDocument: function () {
    if (this.mAssetURL && hostedOnDM()) {
      IWCommentCountForURL(
        this.mAssetURL,
        this.p_commentCountCallback.bind(this)
      );
    } else {
...
var IWMediaStreamPageEntry = Class.create(IWStreamEntry, {
  ...
  positionedThumbnailMarkupForRect: function (rect) {
    return IWMediaStreamPageEntryPositionedThumbnailMarkupForRect(
      this.mThumbnail,
      rect
    );
  },
});
function IWMediaStreamPageEntryPositionedThumbnailMarkupForRect(
  thumbnail,
  rect
) {
  var thumbnailSize = thumbnail.naturalSize();
  var scale = Math.max(
    rect.size.width / thumbnailSize.width,
    rect.size.height / thumbnailSize.height
  );
  var imageSize = thumbnailSize.scale(scale, scale, true);
  var imagePosition = new IWPoint(
    (rect.size.width - imageSize.width) / 2,
    (rect.size.height - imageSize.height) / 2
  );
  imagePosition = imagePosition.scale(1, 1, true);
...
var IWMediaStreamPhotoPageEntry = Class.create(IWMediaStreamPageEntry, {
  ...
  loadThumbnail: function (callback) {
    ...
    var uniqueId =
      "iFrame_" + new Date().getTime() + IWMediaStreamPhotoPageEntryUniqueId;
    loadingArea.innerHTML =
      "<iframe id=" +
      uniqueId +
      ' src="streamloader.html?scriptURL=' +
      this.mStreamScriptURL +
      "&id=" +
      this.mSlideshowId +
  ...
  metric: function () {
    ...
    if (this.mMedia) {
      for (var index = 0; index < this.mMedia.length; ++index) {
        if (this.mMedia[index].isMovie()) ++clipCount;
        else ++photoCount;
      }
    }
  ...
  thumbnailMarkupForRect: function (rect) {
    ...
    if (this.mThumbnail) {
      markup =
        ...
        '<div id="' +
        this.mSlideshowId +
        '-slideshow_placeholder" style="position: absolute; left: 0px; top: 0px; height: 100%; width: 100%; overflow: hidden; ">' +
        "</div>" +
        "</div>";
    }
    return markup;
  ...
  didInsertThumbnailMarkupIntoDocument: function () {
    if (this.mThumbnail) {
      if (isiPhone == false) {
        ...
        prefs["mediaStreamObject"] = {
          load: function (media, baseURL, callback) {
            callback(media);
          }.bind(null, this.mMedia),
        };
        new SlideshowGlue(
          this.mSlideshowId,
...
function IWMediaStreamPhotoPageSetMediaStream(mediaStream, slideshowId) {
  mediaStream.load(
    IWMediaStreamPhotoPageEntryPrefs.baseURL,
    function (slideshowId, media) {
...
var IWMediaStreamMediaPageEntry = Class.create(IWMediaStreamPageEntry, {
  ...
  thumbnailMarkupForRect: function (rect) {
    ...
    var thumbnailMarkup =
      this.positionedThumbnailMarkupForRect(rect) +
      this.badgeMarkupForRect(badgeRect);
    var idAttribute = "";
...
var IWMediaCollection = Class.create(IWFeed, {
  ...
  p_interpretItems: function (baseURL, items) {
    ...
    if (IWMediaStreamPhotoPageSetPrefs) {
      IWMediaStreamPhotoPageSetPrefs({
        slideshowEnabled: this.mSlideshowEnabled,
        fadeIn: true,
    ...
    for (var i = 0; i < items.length; ++i) {
      ...
      if (link != null) {
        ...
        if (skip == false) {
          ...
          var thumbnail =
            this.p_firstElementByTagNameNS(
              item,
              thumbnailNamespace,
              "thumbnail"
          ...
          if (thumbnail) {
            ...
            if (thumbnail.firstChild && thumbnail.firstChild.nodeValue) {
              thumbnailURL =
                thumbnail.firstChild.nodeValue.toRelativeURL(baseURL);
            }
            var contentsText = null;
            var contents = this.p_firstElementByTagNameNS(
              item,
              iWebNamespace,
              "contents"
            );
            ...
            var isMovie = false;
            var isMovieValue = this.p_firstElementByTagNameNS(
              item,
              iWebNamespace,
              "is-movie"
            );
            if (
              isMovieValue &&
              isMovieValue.firstChild &&
              isMovieValue.firstChild.nodeValue
            ) {
              isMovie = isMovieValue.firstChild.nodeValue == "true";
            }
            entry = new IWMediaStreamMediaPageEntry(
              pageURL,
              thumbnailURL,
              titleText,
              contentsText,
            ...
            var pageScriptURL = pageURL
              .stringByDeletingLastPathComponent()
              .stringByAppendingPathComponent(pageName + "_files")
              .stringByAppendingPathComponent(pageName + ".js");
            entry = new IWMediaStreamPhotoPageEntry(
              pageScriptURL,
              pageURL,
              titleText,
              albumContentsFunction,
              guid
            );
          }
  ...
  p_interpretItems: function (baseURL, items) {
    ...
    for (var i = 0; i < items.length; ++i) {
      ...
      if (enclosure != null) {
        ...
        var thumbnailNamespace = "urn:iphoto:property";
        var richTitleHTML;
        var richTitle = this.p_firstElementByTagNameNS(
          item,
          iWebNamespace,
          "richTitle"
        );
        if (
          richTitle &&
          richTitle.firstChild &&
          richTitle.firstChild.nodeValue
        ) {
          richTitleHTML =
            richTitle.firstChild.nodeValue.stringByUnescapingXML();
        }
        ...
        }, "Could not get thumbnail from feed.  Server configuration may have changed.");
        if (thumbnail) {
          var entry = null;
          var type = enclosure.getAttribute("type").split("/");
          var thumbnailURL =
            thumbnail.firstChild.nodeValue.toRebasedURL(baseURL);
          if (type[0] == "video") {
            var movieURL = enclosure.getAttribute("url").toRebasedURL(baseURL);
            var movieParams = null;
            var movieParamsElement = this.p_firstElementByTagNameNS(
              item,
              iWebNamespace,
              "movieParams"
            );
            if (
              movieParamsElement &&
              movieParamsElement.firstChild &&
              movieParamsElement.firstChild.nodeValue
            ) {
              var movieParamsJSON =
                movieParamsElement.firstChild.nodeValue.stringByUnescapingXML();
              movieParams = eval("(" + movieParamsJSON + ")");
            }
            var assetURL = movieURL.stringByDeletingLastPathComponent();
            IWAssert(function () {
              return assetURL != null;
            }, "could not determine asset URL for movie: " + movieURL);
            entry = new IWMovieStreamEntry(
              assetURL,
              this.mShowCommentIndicator,
              movieURL,
              thumbnailURL,
              titleText,
              richTitleHTML,
            ...
            var assetURL = imageURL
              ...
              .stringByDeletingPathExtension();
            var microURL = null;
            var micro = this.p_firstElementByTagNameNS(
              item,
              iWebNamespace,
              "micro"
            );
            ...
            if (!microURL) {
              microURL = (
                imageURL.urlStringByDeletingQueryAndFragment() +
                "?derivative=micro"
              ).toRebasedURL(baseURL);
            }
            var mipThumbnailURL = null;
            var mipThumbnail = this.p_firstElementByTagNameNS(
              item,
              iWebNamespace,
              "mip-thumbnail"
            );
            if (
              mipThumbnail &&
              mipThumbnail.firstChild &&
              mipThumbnail.firstChild.nodeValue
            ) {
              mipThumbnailURL =
                mipThumbnail.firstChild.nodeValue.toRebasedURL(baseURL);
            }
            if (!mipThumbnailURL) {
              mipThumbnailURL = (
                imageURL.urlStringByDeletingQueryAndFragment() +
                "?derivative=mip&alternate=thumb"
              ).toRebasedURL(baseURL);
            }
            IWAssert(function () {
              return assetURL != null;
            }, "could not determine asset URL for image: " + imageURL);
            entry = new IWImageStreamEntry(
              assetURL,
              this.mShowCommentIndicator,
              imageURL,
              thumbnailURL,
...
function setFrameOptionallyMovingContents(element, frame, moveContents) {
  ...
  var offset = new IWPoint(
    oldOrigin.x - frame.origin.x,
    oldOrigin.y - frame.origin.y
  );
  if (moveContents) {
...
var PhotoView = Class.create({
  ...
  initStyle: function () {
    ...
    if (this.canvas) {
      ...
      $(this.canvas).setAttribute(
        ...
        box_size.height -
          Math.max(
            this.reflectionHeight + this.reflectionOffset,
            this.captionHeight
          )
      );
      if (!windowsInternetExplorer) {
  ...
  setImage: function (photo) {
    ...
    if (
      (img_aspect > box_aspect && this.scaleMode == "fit") ||
      (img_aspect < box_aspect && this.scaleMode == "fill")
    ) {
      var ratio = box_size.height / img_size.height;
      if (this.scaleMode == "fit") ratio = Math.min(ratio, 1.0);
    ...
    if (this.reflection) {
      ...
      if (windowsInternetExplorer) {
        ...
        if (w > 0 && this.reflection.height > 0) {
          ...
          var components = this.backgroundColor
            .toLowerCase()
  ...
  setClipPx: function (top, width, height, left) {
    this.div.style.clip =
      "rect(" + top + "px " + width + "px " + height + "px " + left + "px)";
  },
  ...
  setClipToMax: function () {
    ...
    this.div.style.clip =
      "rect(0px " + size.width + "px " + size.height + "px 0px)";
  },
  ...
  p_movieDidFinish: function () {
    NotificationCenter.postNotification(
      new IWNotification("PhotoViewMovieDidEnd", this, {})
    );
  },
  ...
  p_showMovie: function () {
    ...
    if (this.object) {
      ...
      if (!windowsInternetExplorer) {
        ...
        this.object.style.zIndex = "inherit";
      }
      this.p_updateMovieRect();
      var shouldAutoplay =
        this.movieMode == kAutoplayMovie || this.photo.params.movieAutoPlay;
      this.p_appendParamToObject(
        "controller",
        this.photo.params.movieShowController ? "true" : "false"
      );
      this.p_appendParamToObject("autoplay", shouldAutoplay ? "true" : "false");
      this.p_appendParamToObject("scale", "tofit");
      this.p_appendParamToObject("volume", "12800");
      this.p_appendParamToObject(
        "loop",
        this.photo.params.movieLoop != "SFDMovieNoLoop" ? "true" : "false"
      );
      this.p_appendParamToObject(
        "starttime",
        this.p_timeString(
          this.photo.params.movieStartTime * this.photo.params.movieDuration
        )
      );
      this.p_appendParamToObject(
        "endtime",
        this.p_timeString(
          this.photo.params.movieEndTime * this.photo.params.movieDuration
        )
      );
      this.p_appendParamToObject("src", this.movieURL);
      PhotoViewWaitingForDonePlaying[this.movieIndex] =
        this.p_movieDidFinish.bind(this);
      this.p_appendParamToObject(
        "qtnext1",
        "<javascript:PhotoViewDonePlaying(" + this.movieIndex + ");>"
      );
...
var TransitionEffect = Class.create({
  initialize: function (current, next, oncomplete, shouldTighten) {
    ...
    this.effects = [
      { name: "Random", method: "random" },
      { name: "Jump", method: "jump" },
      { name: "Fade", method: "fade" },
      {
        name: "Wipe",
        directions: [
          { name: "Right", method: "wipeRight" },
          { name: "Left", method: "wipeLeft" },
          { name: "Down", method: "wipeDown" },
          { name: "Up", method: "wipeUp" },
          { name: "In", method: "wipeIn" },
          { name: "Out", method: "wipeOut" },
        ],
      },
      {
        name: "Close",
        directions: [
          { name: "Horizontal", method: "wipeCloseHoriz" },
          { name: "Vertical", method: "wipeCloseVert" },
        ],
      },
      {
        name: "Open",
        directions: [
          { name: "Horizontal", method: "wipeOpenHoriz" },
          { name: "Vertical", method: "wipeOpenVert" },
        ],
      },
      {
        name: "Reveal",
        directions: [
          { name: "Right", method: "slideOffRight" },
          { name: "Left", method: "slideOffLeft" },
          { name: "Down", method: "slideOffDown" },
          { name: "Up", method: "slideOffUp" },
        ],
      },
      {
        name: "Slide On",
        directions: [
          { name: "Right", method: "slideOnRight" },
          { name: "Left", method: "slideOnLeft" },
          { name: "Down", method: "slideOnDown" },
          { name: "Up", method: "slideOnUp" },
        ],
      },
      {
        name: "Push",
        directions: [
          { name: "Right", method: "pushRight" },
          { name: "Left", method: "pushLeft" },
          { name: "Down", method: "pushDown" },
          { name: "Up", method: "pushUp" },
        ],
      },
  ...
  tighten: function (horiz, vert) {
    if (this.shouldTighten && (vert || horiz)) {
      ...
      this.current.originalFrame = new IWRect(
        this.current.div.offsetLeft,
        this.current.div.offsetTop,
        this.current.div.offsetWidth,
        this.current.div.offsetHeight
      );
      this.next.originalFrame = new IWRect(
        this.next.div.offsetLeft,
        this.next.div.offsetTop,
        this.next.div.offsetWidth,
        this.next.div.offsetHeight
      );
      this.current.offset = setFrameOptionallyMovingContents(
        this.current.div,
        frame,
        true
      );
      this.next.offset = setFrameOptionallyMovingContents(
        this.next.div,
        frame,
        true
      );
  ...
  loosen: function (loosenHoriz, loosenVert) {
    if (this.shouldTighten && (loosenHoriz || loosenVert)) {
      setFrameOptionallyMovingContents(
        this.current.div,
        this.current.originalFrame,
        false
      );
      setFrameOptionallyMovingContents(
        this.next.div,
        this.next.originalFrame,
        false
      );
  ...
  fade: function () {
    this.stop();
    var self = this;
    this.animation = new SimpleAnimation(function () {
      self.animationComplete();
    });
  ...
  slideOffRight: function () {
    this.doWipe(
      function (now) {
        var x = now * this.width;
        this.current.setLeftPx(
          x + (this.width - parseInt(this.current.div.style.width)) / 2
        );
      },
      false,
      true,
    ...
    this.doWipe(
      function (now) {
        ...
        this.next.setLeftPx(
          x -
            this.width +
            (this.width - parseInt(this.current.div.style.width)) / 2
        );
      },
      false,
      true,
...
var Slideshow = Class.create({
  ...
  p_setAdvanceAnchorHandler: function () {
    if (this.advanceAnchor) {
      if (this.current.photo && this.current.photo.movieURL) {
        ...
        this.advanceAnchor.onclick = function () {
          return false;
        };
      } else {
  ...
  p_showPhotoNumber: function (number, skipTransition, override) {
    ...
    if (next.photo.image.loaded() == false) {
      next.photo.image.load(
        arguments.callee.bind(this, number, skipTransition, false)
      );
      return;
    }
    ...
    this.transition = new TransitionEffect(
      ...
      next.view,
      function () {
        self.transitionComplete(number);
      },
      this.shouldTighten
    );
    if (
      skipTransition ||
      new Date().getTime() - this.lastPhotoChange.getTime() < transitionTime
    ) {
    ...
    if (next === prev) {
      ...
      this.prev.photo.image.load(
        this.prev.view.setImage.bind(this.prev.view, this.prev.photo)
      );
    } else {
      this.prev = current;
      this.next = prev;
      this.next.photo = this.photos[this.nextPhotoNumber()];
      this.next.photo.image.load(
        this.next.view.setImage.bind(this.next.view, this.next.photo)
      );
    }
  ...
  startNextPhotoTimer: function (time) {
    ...
    if (this.nextPhotoTimer != null) {
      (function () {
        return false;
      }).assert("trying to set overlapping timer");
      return;
    }
    var self = this;
  ...
  setImage: function (image) {
    ...
    current.photo.image.load(
      current.view.setImage.bind(current.view, current.photo)
    );
  },
...
function IWStartSlideshow(url, imageStream, index, fullScreen) {
  ...
  var slideWindow = window.open(
    url,
    "slideshow",
    "scrollbars=no,titlebar=no,location=no,status=no,toolbar=no,resizable=no,width=" +
      windowWidth +
      ",height=" +
      windowHeight +
      ",top=" +
...
function IWSetSpacerHeight(photogrid, height) {
  ...
  var spacerHeight = Math.max(
    parseInt(photogrid.style.top) + height - spacer.offsetTop,
    initialSpacerHeight
  );
  spacer.style.height = spacer.style.lineHeight = px(spacerHeight);
}
function IWLayoutPhotoGrid(
  divID,
  layout,
  stroke,
  imageStream,
  ...
  if (isiPhone) {
    slideshowURL = slideshowURL
      .stringByDeletingLastPathComponent()
      .stringByAppendingPathComponent("phoneshow.html");
  }
  slideshowURL =
    slideshowURL.stringByAppendingAsQueryString(slideshowParameters);
  if (detailViewID != null) {
    ...
    if (!detailView.preferenceForKey("mediaStreamObject")) {
      ...
      NotificationCenter.addObserver(
        null,
        toggleDetailViewNotification,
        DetailViewToggleNotification,
        divID
      );
      if (slideshowParameters) {
        detailView.setPlaySlideshowFunction(
          IWStartSlideshow.bind(
            null,
            slideshowURL,
            imageStream,
            0,
            slideshowParameters["fullScreen"]
          )
        );
      }
    }
  ...
  for (var i = 0; i < range.length(); ++i) {
    ...
    frame.style.cssText = iWPosition(
      true,
      translation.x,
      translation.y,
      size.width,
  ...
  var renderThumb = function (i, streamIndex) {
    ...
    if (textBoxSize.height > 0) {
      ...
      var opacityMarkup = "";
      if (opacity < 0.999) {
        opacityMarkup = windowsInternetExplorer
    ...
    if (
      Math.abs(scaledImageSize.width - thumbnailSize.width) <= 2 &&
      Math.abs(scaledImageSize.height - thumbnailSize.height) <= 2
    ) {
      scaledImageSize = thumbnailSize;
    }
    var frameMarkup = stroke.markupForImageStreamEntry(
      imageStreamEntry,
      scaledImageSize
    );
    ...
    var framePos = layout.translationForImageOfSizeAtIndexWithOffset(
      new IWSize(
        parseFloat(framed.style.width || 0),
        parseFloat(framed.style.height || 0)
      ),
      i,
      null
    );
    ...
    if (detailViewID) {
      ...
      anchor.onclick = IWToggleDetailView.bind(
        null,
        true,
        streamIndex,
        divID,
    ...
    if (shadow) {
      if (windowsInternetExplorer) {
        ...
        var blurRadius = shadow.mBlurRadius * 0.75;
        ...
        shadowDiv.style.filter =
          "progid:DXImageTransform.Microsoft.MaskFilter()" +
          " progid:DXImageTransform.Microsoft.MaskFilter(color=" +
          shadow.mColor +
          ")" +
  ...
  for (var i = 0; i < range.length(); ++i) {
    ...
    imageStream[streamIndex].loadThumbnail(
      IWJobQueue.prototype.addJob.bind(
        IWJobQueue.sharedJobQueue,
        renderThumbJob
      )
    );
  }
  ...
  if (headerView) {
    if (slideshowParameters) {
      headerView.setPlaySlideshowFunction(
        IWStartSlideshow.bind(
          null,
          slideshowURL,
          imageStream,
...
var IWPhotoGridLayout = Class.create({
  ...
  translationForTileAtIndex: function (index) {
    ...
    var translation = new IWPoint(
      tileSize.width * (index % columnCount) +
        IWPhotoGridLayoutConstants.kBorderThickness,
      tileSize.height * Math.floor(index / columnCount) +
        IWPhotoGridLayoutConstants.kBorderThickness +
        this.mTopPadding +
        IWPhotoGridLayoutConstants.vTopSpacing
    );
    return translation;
  },
  translationForImageOfSizeAtIndexWithOffset: function (
    imageSize,
    index,
    offset
  ) {
    ...
    var imageAreaSize = new IWSize(
      tileSize.width,
      tileSize.height -
        (IWPhotoGridLayoutConstants.vTextOffsetFromImage +
          this.mTextBoxSize.height +
          IWPhotoGridLayoutConstants.vTextOffsetFromBottom)
    );
  ...
  translationForTextAtIndexWithOffset: function (index, offset) {
    ...
    var translation = new IWPoint(
      (tileSize.width - textBoxSize.width) / 2.0,
      tileSize.height -
        textBoxSize.height -
        IWPhotoGridLayoutConstants.vTextOffsetFromBottom
    );
  ...
  totalHeightForCount: function (count) {
    ...
    var tileSize = this.mTileSize;
    return (
      tileSize.height * lastRow +
      IWPhotoGridLayoutConstants.kBorderThickness * 2 +
      this.mTopPadding +
      this.mBottomPadding
    );
  },
  p_scaleForImageOfSize: function (imageSize) {
    ...
    return Math.min(
      (maxImageSize.width - this.mFramePadding.width) / imageSize.width,
